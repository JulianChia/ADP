...
class Table(ttk.PanedWindow):
    ...
    By self.tree:
    "<<TreePopulateDone>>" - after self.tree is first populated with items.
    ...
    icons:
    <a href="https://www.flaticon.com/free-icons/user-interface" title="user interface icons"> User interface icons created by Metami septiana - Flaticon</a>/
    <img src="https://icons.iconarchive.com/icons/oxygen-icons.org/oxygen/16/Actions-edit-delete-icon.png" width="16" height="16">
    """
    ...
    def __init__(self, master, **options):
        ...
        i1 = str(CWD) + "/icons/copy.png"
        i2 = str(CWD) + "/icons/delete.png"
    ...
    def set_sdir(self, directory: tk.StringVar):
        """Method to define the directory to search for duplicated pictures."""
        self.sdir = directory
    ...
    def _create_widgets(self):
        ...
        self.table = ttk.Frame(self, style="TableFrame.TFrame")
        self.add(self.table)
        self.create_tree_with_scrollbars()
        self.create_buttons()

    def create_tree_with_scrollbars(self):
        self.tree = ttk.Treeview(self.table, height=10, selectmode='extended',
        ...
        self.ysb = AutoScrollbar(self.table, orient='vertical',
                                 command=self.tree.yview)
        self.xsb = AutoScrollbar(self.table, orient='horizontal',
    ...
    def create_buttons(self):
        ...
        # A row of buttons
        self.framebns = ttk.Frame(self.table, style='Framebns.TFrame')
    ...
    def populate_tree_the_first_time(self):
        ...
        if spage[1] == 0:
            ...
            for n, page in enumerate(self.shown_pages):
                if page >= 0:
                    self.shown_giids[n] = \
                        self.sql3db.get_group_ids_of_page(page)
                else:
                    self.shown_giids[n] = []

            # 2.4. Generate virtual event immediately
            self.tree.event_generate("<<TreePopulateDone>>")
    ...
    def _bn_toggle_dtype(self, dtype):
        ...
        # 2.1 Get the iids of all dtype pictures attached to the Treeview widget
    ...
    def update_bn_delete_state(self):
        if not self.sql3db.get_selected():
            self.bn_delete.state(["disabled"])
        else:
            self.bn_delete.state(["!disabled"])

    def _delete_button_invoked(self):
        """Callback to delete the selected picture files."""
        ...
        # 4. Delete the selected picture files
        start = perf_counter()
        ...
        dtime = end - start
        print(f"*** {len(selected)} PICTURES DELETED in {dtime:.6f} secs.! ***")
        ...
        if len(all_pages) == 1:  # If only 1 page
            ...
            if self.shown_pages[2] == all_pages[-1]:  # A. Next page is last page
                if ysb_btm == 1.0:  # A.1. At last page bottom
                    ...
                    # Generate virtual event to trigger a followup process to
                    # ensure the Dupgroup of the first visible group item of
                    # the Treeview is at the top of the self.viewport in the
                    # Gallery widget.
                    tree.event_generate("<<TreeScrollDownDone>>", when="tail")
                ...
                #     to the next page
                vf_in_npage = any(
                    [fiid in npage_fiids for fiid in visible_fiids])
                if visible_giids:
                    ...
                    if vg_in_npage and vf_in_npage:
                        # Create or reattach next next page
                        self.create_or_reattach_next_next_page(visible_giids,
                                                               visible_fiids)
                    else:  # B.2.F Next page group and file items aren't visible.
                        # Generate virtual event to trigger a followup
                        # process to ensure the Dupgroup of the first visible
                        # group item of the Treeview is at the top of the
                        # self.viewport in the Gallery widget.
                        tree.event_generate("<<TreeScrollDownDone>>",
                                            when="tail")
                else:
                    # B.2.T Only next page file items are visible.
                    if vf_in_npage:
                        # Create or reattach next next page
                        self.create_or_reattach_next_next_page(visible_giids,
                                                               visible_fiids)
    ...
    def _tree_show_previous_previous_page(self):
        ...
        if len(all_pages) == 1:  # If only 1 page
            ...
            if self.shown_pages[0] == all_pages[0]:  # A. Previous page is first page
                ...
                vf_in_ppage = any(
                    [fiid in ppage_fiids for fiid in visible_fiids])
                if visible_giids:
                    vg_in_ppage = any(
                    ...
                    if vg_in_ppage and vf_in_ppage:
                        self.reattach_previous_previous_page(visible_giids,
                                                             visible_fiids)
                    else:  # B.2.F Next page group and file items aren't visible.
                        # Generate virtual event to trigger a followup
                        # process to ensure the Dupgroup of the first visible
                    ...
                    # B.2.T Only previous page file items are visible.
                    if vf_in_ppage:
                        self.reattach_previous_previous_page(visible_giids,
                                                             visible_fiids)
                    else:  # B.2.F Next page group and file items aren't visible.
    ...
    def update_tree_file_item_tags(self, fiid):
    ...
    def _create_bindings(self):
        ...
        if platform.system() in "Linux":
            ...
            tree.bind("<MouseWheel>", self._event_on_mousewheel)
            ysb.bind("<MouseWheel>", self._event_on_mousewheel)

        # Control how self.tree show next next page and previous previous page
        tree.bind("<<TreeScrollDown>>", self._event_tree_show_next_next_page)
        tree.bind("<<TreeScrollUp>>",
                  self._event_tree_show_previous_previous_page)
    ...
    def _event_on_mousewheel(self, event):
        """Event handler to manage Mousewheel rotation for Mac and Windows OS.
        """
        if platform.system() in "Darwin":
            if event.delta < 0:
                self._event_schedule_scroll_down(event)
            elif event.delta > 0:
                self._event_schedule_scroll_up(event)
    ...
    def _event_schedule_scroll_down(self, event):
        ...
        if self.populated and not self.is_updating:
    ...
    def _event_tree_show_next_next_page(self, event):
        self._tree_show_next_next_page()

    def _event_tree_show_previous_previous_page(self, event):
        self._tree_show_previous_previous_page()
    ...
    def _event_bind_xsb(self, event):
        if platform.system() in "Linux":
            ...
            event.widget.bind('<Enter>',
                              self._event_bind_xsb_to_mousewheel_macos)
            event.widget.bind('<Leave>',
                              self._event_unbind_xsb_from_mousewheel)
        elif platform.system() in "Windows":
            event.widget.bind('<Enter>',
    ...
    def _event_bind_xsb_to_mousewheel_linuxos(self, event):
        ...
        event.widget.bind('<Button-4>', tree_scroll_right)
        event.widget.bind('<Button-5>', tree_scroll_left)
    ...
    def _event_bind_xsb_to_mousewheel_winos(self, event):
        ...
        def scroll_xsb(event):
            print(f"{event.widget=} {event.delta=} {event.num=}")
            self.tree.xview_scroll(int(-10 * event.delta/120), "units")

        event.widget.bind("<MouseWheel>", scroll_xsb)

    def _event_bind_xsb_to_mousewheel_macos(self, event):

        def scroll_xsb(event):
            print(f"{event.widget=} {event.delta=} {event.num=}")
            self.tree.xview_scroll(int(-10 * event.delta), "units")

        event.widget.bind("<MouseWheel>", scroll_xsb)

    def _event_unbind_xsb_from_mousewheel(self, event):
    ...
    def _event_b1_release_to_toggle_selection(self, event):
        ...
        # 1. Do nothing in these scenarios
        #    - if widget_under_pointer != event.widget
        #    - if tree is empty
        #    - if Table is updating
        ...
        # 2. Tasks for all other scenarios
        tree = self.tree
        db = self.sql3db
...
if __name__ == '__main__':
    ...
    class App(ttk.Frame):
        def __init__(self, master, **options):
            ...
            try:
                ...
                if layout in ["horizontal", "vertical"]:
                    ...
                    raise ValueError(
                        f"layout={layout} is invalid. It's value must "
        ...
        def event_reset_app(self, event):
            ...
            self.find.bn_find.instate(["disabled"],
